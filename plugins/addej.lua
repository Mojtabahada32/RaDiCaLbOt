...
local function ToOfan(msg, parts)
	if is_mod(msg) then
		local hash = "gp_lang:"..msg.to.id
		local lang = redis:get(hash)
		if parts[1]:lower() == 'ادد اجباری' then
			if parts[2]:lower() == 'غیرفعال' then
		...
		if parts[1]:lower() == 'تنظیم محدودیت' and parts[2] then
			if tonumber(parts[2]) >= 1 and tonumber(parts[2]) <= 50 then
				...
				if not lang then
					return '_Range Number is between_ *1 - 50*'
				else
					return '_تعداد باید مابین_ `1 - 50` _باشد_'
				end
			end
		end
		if parts[1]:lower() == 'نمایش' then
		...
		if parts[1]:lower() == 'پیام محدودیت' then
			local hsh = 'addpm'..msg.to.id
			if parts[2] == 'فعال' then
				redis:del(hsh)
				if not lang then
